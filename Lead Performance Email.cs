...
namespace Lead_Performance_Email {
  class Program {
    enum TIMEPERIOD: int {
      currentMonth = 0, previousMonth = 1, currentFYTD = 2, previousFYTD = 3
    };
    enum RESULTSET: int {
      first = 0, second = 1
    };
    ...
    enum METRIC: int {
      valids = 0, uniques = 1, sales = 2, closeRate = 3
    };
    ...
    static void Emailer(DataSet[] queryResults, string subjectLine, string[] emailDestinations) {
      ...
      SmtpClient smtp_server = new SmtpClient("SMTPCLIENT");
        MailMessage email = new MailMessage(); email.AlternateViews.Add(alternate);
